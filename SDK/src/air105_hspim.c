...
void HSPIM_Init(HSPIM_InitTypeDef* HSPIM_InitStruct) {
    ...
    HSPIM->CR1 = (uint32_t)((HSPIM->CR1 & HSPIM_CR1_CLEAR_MASK) | ((HSPIM_InitStruct->HSPIM_BaudRatePrescaler & 0x1FF) << HSPIM_CR1_PARAM_BAUDRATE_POS) | ((HSPIM_InitStruct->HSPIM_ReceiveDataLength & 0x3FF) << HSPIM_CR1_PARAM_RECEIVE_DATA_LENGTH_POS));

    HSPIM->FCR = (uint32_t)((HSPIM->FCR & HSPIM_FCR_CLEAR_MASK) | ((HSPIM_InitStruct->HSPIM_TXFIFOEmptyThreshold & 0x3F) << HSPIM_FCR_PARAM_TRANSIMIT_FIFO_EMPTY_THRESHOULD_POS) | ((HSPIM_InitStruct->HSPIM_RXFIFOFullThreshold & 0x3F) << HSPIM_FCR_PARAM_RECEIVE_FIFO_FULL_THRESHOULD_POS));

    HSPIM->CR0 = (uint32_t)((HSPIM->CR0 & HSPIM_CR0_CLEAR_MASK) | ((HSPIM_InitStruct->HSPIM_CPOL & 0x01) << HSPIM_CR0_PARAM_CPOL_POS) | ((HSPIM_InitStruct->HSPIM_CPHA & 0x01) << HSPIM_CR0_PARAM_CPHA_POS) | ((HSPIM_InitStruct->HSPIM_FirstBit & 0x01) << HSPIM_CR0_PARAM_FIRST_BIT_POS) | ((HSPIM_InitStruct->HSPIM_Mode & 0x07) << HSPIM_CR0_PARAM_MODEL_SELECT_POS) | ((HSPIM_InitStruct->HSPIM_DivideEnable & 0x01) << HSPIM_CR0_PARAM_DIVIDE_ENABLE_POS) | ((HSPIM_InitStruct->HSPIM_Enable & 0x01) << HSPIM_CR0_PARAM_ENABLE_POS));
}
...
void HSPIM_DMAInit(HSPIM_DMAInitTypeDef* HSPIM_DMAInitStruct) {
    if (ENABLE == HSPIM_DMAInitStruct->HSPIM_DMAReceiveEnable) {
        ...
        HSPIM->DCR = (uint32_t)((HSPIM->DCR & HSPIM_DCR_RECEIVE_LEVEL_CLEAR_MASK) | ((HSPIM_DMAInitStruct->HSPIM_DMAReceiveLevel) << HSPIM_DCR_PARAM_DMA_RECEIVE_LEVEL_POS));
    ...
    if (ENABLE == HSPIM_DMAInitStruct->HSPIM_DMATransmitEnable) {
        ...
        HSPIM->DCR = (uint32_t)((HSPIM->DCR & HSPIM_DCR_TRANSMIT_LEVEL_CLEAR_MASK) | ((HSPIM_DMAInitStruct->HSPIM_DMATransmitLevel) << HSPIM_DCR_PARAM_DMA_TRANSMIT_LEVEL_POS));
...
void HSPIM_ModeSwitch(uint8_t Mode) {
    ...
    HSPIM->CR0 = (uint32_t)((HSPIM->CR0 & HSPIM_CR0_MODE_SELECT_CLEAR_MASK) | ((Mode & 0x07) << HSPIM_CR0_PARAM_MODEL_SELECT_POS));
