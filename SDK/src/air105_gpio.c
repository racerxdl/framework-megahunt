...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct) {
    ...
    switch ((uint32_t)GPIO_InitStruct->GPIO_Mode) {
        case GPIO_Mode_IN_FLOATING: {
            ...
            GPIOx->PUE &= ~GPIO_InitStruct->GPIO_Pin;  // Pull Up Disable
            break;
        }
        case GPIO_Mode_IPU: {
            GPIOx->OEN |= GPIO_InitStruct->GPIO_Pin;  // Input Mode
            GPIOx->PUE |= GPIO_InitStruct->GPIO_Pin;  // Pull Up Enable
            break;
        }
...
void GPIO_SetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
    ...
    if (GPIO_OD_Mark[GPIONum] & GPIO_Pin) {
        // input bits -> filtered by OD mask -> only OD bits remains -> apply to OEN with OR
        GPIOx->OEN |= ((uint32_t)(GPIO_OD_Mark[GPIONum] & GPIO_Pin));
    }
    if ((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin) {
        // input bits -> filtered by comp bits of OD mask -> only non-OD bits remains -> apply to BSRR
        GPIOx->BSRR = ((uint32_t)((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin));
    }
}
...
void GPIO_ResetBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
    ...
    if (GPIO_OD_Mark[GPIONum] & GPIO_Pin) {
        // input bits -> filtered by OD mask -> only OD bits remains -> comp bits -> apply to OEN with AND
        GPIOx->OEN &= ~((uint32_t)(GPIO_OD_Mark[GPIONum] & GPIO_Pin));
    }
    if ((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin) {
        // input bits -> filtered by comp bits of OD mask -> only non-OD bits remains -> apply to high 16-bit of BSRR
        GPIOx->BSRR = (((uint32_t)((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin)) << 16);
    }
}
...
void GPIO_ToggleBits(GPIO_TypeDef* GPIOx, uint16_t GPIO_Pin) {
    ...
    if (GPIO_OD_Mark[GPIONum] & GPIO_Pin) {
        // input bits -> filtered by OD mask -> only OD bits remains -> apply to OEN with XOR
        GPIOx->OEN ^= ((uint32_t)(GPIO_OD_Mark[GPIONum] & GPIO_Pin));
    }
    if ((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin) {
        // input bits -> filtered by comp bits of OD mask -> only non-OD bits remains -> apply to IODR with XOR
        GPIOx->IODR ^= (uint32_t)((~GPIO_OD_Mark[GPIONum]) & GPIO_Pin);
    }
}
