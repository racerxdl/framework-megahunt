...
static MH_StatusTypeDef MH_QSPI_Command(MH_CommandTypeDef *cmd, int32_t timeout) {
    ...
    AIR105_MODIFY_REG32(&(QSPI->REG_WDATA), (QUADSPI_REG_WDATA), (cmd->WrData));
    AIR105_MODIFY_REG32(&(QSPI->ADDRES), (QUADSPI_ADDRESS_ADR), (cmd->Address << 8));
    AIR105_MODIFY_REG32(&(QSPI->FCU_CMD),
    ...
    AIR105_WRITE_REG32(&(cmd->RdData), QSPI->REG_RDATA);
    return status;
}
...
void QSPI_Init(QSPI_InitTypeDef *mhqspi) {
    if (mhqspi == NULL) {
        ...
        QSPI->CACHE_INTF_CMD = (uint32_t)((mhqspi->Cache_Cmd_ReleaseDeepInstruction << 24) | (mhqspi->Cache_Cmd_DeepInstruction << 16) | ((mhqspi->Cache_Cmd_ReadBusMode & 0x03) << 12) | ((mhqspi->Cache_Cmd_ReadFormat & 0x0F) << 8) | (mhqspi->Cache_Cmd_ReadInstruction));

        QSPI->DEVICE_PARA = (uint32_t)((QSPI->DEVICE_PARA & ~0xFFFF) | (((mhqspi->SampleDly & 0x01) << QSPI_DEVICE_PARA_SAMPLE_DLY_Pos) | ((mhqspi->SamplePha & 0x01) << QSPI_DEVICE_PARA_SAMPLE_PHA_Pos) | ((mhqspi->ProToCol & 0x03) << QSPI_DEVICE_PARA_PROTOCOL_Pos) | ((mhqspi->DummyCycles & 0x0F) << QSPI_DEVICE_PARA_DUMMY_CYCLE_Pos) | ((mhqspi->FreqSel & 0x03)) | QSPI_DEVICE_PARA_FLASH_READY_Mask));
    }
}
